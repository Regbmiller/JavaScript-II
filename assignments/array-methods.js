...
let fullNames = runners.map((runners) => {
  return {'firstName': runners.first_name, 'lastName': runners.last_name}; 
});
...
let firstNamesAllCaps = runners.map((runners) => {
  return {'firstName': runners.first_name.toUpperCase()}
});
...
let ticketPriceTotal = runners.reduce((ticketPriceTotal, state) => {
   return ticketPriceTotal += state.donation;
}, 0);
